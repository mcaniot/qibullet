...
class AgentSpec(object):
    ...
    def __init__(self, id, entry_point=None, kwargs=None):
        ...
        if not match:
            raise\
                 error.Error(
                  'Attempted to register malformed agent ID: {}.'
                  '(Currently all IDs must be of the form {}.)'.format(
                      id,
                      agent_id_re.pattern))
    ...
    def make(self, **kwargs):
        ...
        if self._entry_point is None:
            raise error.Error(
                'Attempting to make deprecated agent {}.'
...
class AgentRegistry(object):
    ...
    def spec(self, id):
        ...
        if not match:
            raise error.Error(
                'Attempted to look up malformed agent ID: {}.'
                '(Currently all IDs must be of the form {}.)'.format(
                    id.encode('utf-8'), agent_id_re.pattern))
        ...
        try:
            ...
            matching_agents = [valid_agent_name for valid_agent_name,
                               valid_agent_spec in self.agent_specs.items()
                               if agent_name == valid_agent_spec._agent_name]
            if matching_agents:
                raise error.DeprecatedEnv(
                    'Agent {}'
                    ' not found (valid versions include {})'.format(
                        id, matching_agents))
            else:
                raise error.UnregisteredEnv(
                    'No registered agent with id: {}'.format(id))
